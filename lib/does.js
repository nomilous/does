  ...
  if (mode === 'spec') {
    lastInstance = local = {
      ...
      */
      ...
      subscriber: function(_arg) {
        var data, event, source;
        source = _arg.source, event = _arg.event, data = _arg.data;
        if (event !== 'test end') {
          return;
        }
        return console.log({
      ...
      assert: deferred(function(action, done) {
        var call, error, expect, expected, expects, fnName, functions, object, original, resulted, spectator, type, uuid, _ref, _ref1;
        if (done == null) {
          done = null;
        }
        if (typeof done === 'function') {
          ...
          for (uuid in local.expectations) {
            ...
            for (fnName in functions) {
              ...
              expect = expects[0];
              call = "" + type + "." + fnName + "()";
              expected[uuid].FuctionExpectations[call] = {
                'was called': true
              };
              resulted[uuid].FuctionExpectations[call] = {
