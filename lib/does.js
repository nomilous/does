    ...
    lastInstance = local = {
      ...
      */
      ...
      activate: function(runtime) {
        var ancestors, original, parent, rname, tapTimeout, _base, _base1;
      ...
      reset: deferred(function(action) {
        var creator, entities, expectation, expects, functionName, functions, functionsCount, holding, name, object, original, uuid, _ref, _ref1, _ref2;
        entities = local.entities;
        local.runtime.holding.length = 0;
        holding = local.runtime.holding;
        for (uuid in entities) {
          _ref = entities[uuid], name = _ref.name, object = _ref.object, functionsCount = _ref.functionsCount, functions = _ref.functions;
      ...
      expectFn: function(_arg) {
        var creator, entity, expect, expectation, expects, fn, fnName, functions, object, original, spectator, spy, stub, tagged, type, uuid, _ref;
        creator = _arg.creator, uuid = _arg.uuid, fnName = _arg.fnName, fn = _arg.fn, spy = _arg.spy;
        entity = local.entities[uuid];
        object = entity.object, type = entity.type, tagged = entity.tagged, spectator = entity.spectator, functions = entity.functions;
      ...
      assert: deferred(function(action, done) {
        var call, error, expect, expectation, expected, expects, fnName, functions, functionsCount, name, object, original, resulted, spec, spectator, type, uuid, _ref, _ref1;
        if (done == null) {
          done = null;
        }
        ...
        if (typeof done === 'function') {
          ...
          for (uuid in local.entities) {
            _ref = local.entities[uuid], expectation = _ref.expectation, object = _ref.object, type = _ref.type, name = _ref.name, spectator = _ref.spectator, functionsCount = _ref.functionsCount, functions = _ref.functions;
            if (!(functionsCount > 0)) {
              continue;
            }
            expected[name] = {
